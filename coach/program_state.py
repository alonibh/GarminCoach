...
def reconcile_active_program(session: Session) -> int:
    ...
    for activity in activities:
        ...
        matched += 1
        session.flush()
        # A match is an explicit local prerequisite event.  Leave it in the
        # dirty journal so sync's one bounded drain can merge this cause with
        # set resolution and record one material notification boundary.
