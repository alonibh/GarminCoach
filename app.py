...
def get_onboarding(request: Request):
    ...
    if config.MULTI_USER_ENABLED:
        user = getattr(request.state, "user", None)
        if user:
            # Active and fully onboarded → go straight to the dashboard
            if user.status == "active" and user.onboarding_step == "complete":
                return RedirectResponse("/", status_code=303)
            # Still onboarding (or active but incomplete) → show the wizard
