...
7. **6B3B3**: in progress: operator apply CLI (`apply_verified_restore.py`) implemented and tested. Requires all four confirmation arguments (`--backup-id`, `--expected-current-commit`, `--confirm-target-set-hash`, `--confirm-restore`); supports explicit re-entry via `--operation-id`; emits bounded versioned JSON to stdout on success and to stderr on error; maps exceptions to stable distinct exit codes 0 and 64-72 by type only; never calls `os.replace`, `os.unlink`, starts/stops service, or runs migrations; delegates all database mutation to the approved Phase 6B3B1 and 6B3B2 APIs; refuses `FAILED_MANUAL_RECOVERY_REQUIRED` re-entry; `COMPLETED` re-entry verifies via engine before returning success. **Immutable backup binding additions (Gate A):** `safety_backup_manifest_sha256` persisted in journal at Phase 6B3B1; every later safety-backup load validated against journal value; `_verify_file_owned()` is fully race-complete (seven facts from fstat and no-follow pathname stat, before and after hashing; descriptor-close failure surfaced); service-stopped error messages are bounded and sanitized; `ROLLED_BACK` re-entry freshly proves every target (actively rolled-back against safety bytes; never-replaced against destination baseline). Old journals missing `safety_backup_manifest_sha256` fail closed: REPLACEMENT_READY requires a new prepare; post-mutation stages report manual recovery. No production restore drill has been performed.
