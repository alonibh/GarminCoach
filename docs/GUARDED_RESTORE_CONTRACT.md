...
5. **6B3B1**: complete: configured-runtime restore preparation through `REPLACEMENT_READY` (`guarded_restore_configured.py`, `guarded_restore_configured_staging.py`, and deterministic tests). It requires mandatory destination-baseline evidence, proves the durable parent identity before and after staging boundaries, strictly validates `STAGED_VERIFIED` and `REPLACEMENT_READY` re-entry, finalizes permissions descriptor-bound with `fchmod`, never applies pathname `chmod` after staged publication, validates exact child sets and binding/artifact ownership, and rereads the destination baseline SHA-256 after staging. Configured databases and WAL/SHM sidecars remain unmodified. No database replacement, rollback, or apply CLI is included.
