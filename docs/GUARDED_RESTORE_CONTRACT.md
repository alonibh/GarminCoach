...
6. **6B3B2**: in progress: configured replacement, postcheck, automatic rollback, safe re-entry, and manual-recovery boundary (`guarded_restore_configured_replacement.py` and deterministic tests). Implements `replace_and_verify_configured_restore` with a keyword-only API accepting no arbitrary paths. Acquires ProcessLock→RestoreLock→BackupLock non-blockingly and holds all three throughout the entire mutation window. Runs a full pre-mutation proof barrier (project root, HEAD commit, package identity, selected and safety backup manifest/file hashes, runtime mode, canonical target set, control-to-tenant mapping, schema fingerprints, migration markers, destination baseline evidence, staging artifact ownership and content, same-filesystem requirement, confirmation and target-set hash). Stages verified rollback artifacts from the safety backup in per-target owned directories (0700) with exclusive no-follow creation, fsynced descriptor-bound permission finalization, canonical binding, single-link ownership, and complete SQLite/schema/marker verification. Replacement order is mandatory: data/tenant targets first, control database last. Before each replacement: reload journal, revalidate destination parent, verify staged and rollback artifact ownership. Named WAL/SHM sidecars handled with lstat/no-follow semantics, durably recorded, never wildcard-deleted. Atomic `os.replace` only; durable `replacement_intent` before and `replacement_completed` after each replacement with post-replacement descriptor-bound verification (type, mode, link count, size, SHA-256, SQLite checks). Complete postcheck before COMPLETED: target discovery, set/mode/mapping, size/SHA, SQLite (quick, integrity, foreign key), schema fingerprint, migration markers, private modes, no WAL/SHM sidecars, backup integrity. Automatic verified rollback on any post-mutation failure: reverse order (control first), pre-verified rollback artifacts only, `os.replace` without unlink, exact safety bytes verified after each rollback, full SQLite/schema/marker checks, fsynced. Legal unambiguous re-entry for REPLACEMENT_READY, REPLACING, REPLACED, POSTCHECK_PASSED, ROLLBACK_REQUIRED, ROLLED_BACK, COMPLETED, FAILED_SAFE. FAILED_MANUAL_RECOVERY_REQUIRED on ambiguous state, conflicting journal facts, or any rollback uncertainty. Post-mutation stage re-entries load backup snapshots without `against_current_config` checks since the current DB state may differ from the original baseline. Strict owned-evidence cleanup (no recursive rmtree, no wildcards) after COMPLETED and after verified FAILED_SAFE. No apply CLI included.
