...
4. **Service Awareness**: Phase 6B3A planning tools do not touch running services. The Phase 6B3B apply CLI (`apply_verified_restore.py`) requires the application service (e.g. systemd `garmincoach.service`) to be stopped manually before invocation. The CLI itself never invokes `systemctl`, starts, stops, or restarts services, deploys code, or runs migrations.
