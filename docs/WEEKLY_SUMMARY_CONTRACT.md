...
The immutable payload `week_end` is the athlete-local scheduled Saturday. The report covers exactly `week_end - 6 days` through `week_end`, inclusive: activity and planned-session timestamps are compared to local `00:00:00` through `23:59:59.999999`. `week_end` is an exact `date`; generation receives a naive athlete-local datetime and rejects future weeks. Retries keep the same payload and may reread newly synced local facts only inside that window.
...
Program completion counts distinct matched activity IDs by activity local start time, only for the current active program. The target is positive `days_per_week`, otherwise non-addon `coach_strength` sessions, otherwise unknown. Normal planned sessions in the window whose status is not in the shared terminal-status vocabulary are described as incomplete. Unmatched strength/weight activities are distinct weekly activities with no current program match. All activity counts include matched and unmatched rows; valid non-negative duration is summed only when observed. Domains are bounded to four and stable-sorted by count then label.
...
`send_weekly_summary()` only enqueues the established `weekly:<date>` durable identity. The existing minute outbox poller is the sole delivery path and materializes the report at delivery as `(text, None, None)`, requiring plain Telegram transport with no reply markup. The row is cancelled for a bad, future, or delivery-date-stale payload (`local date > week_end + 6 days`); retries within that boundary retain the original week. Existing leases, quiet hours, attempt limits, and idempotency remain unchanged.

## Acceptance boundaries

All calculations are tenant-session-local, bounded, deterministic, and read-only. The summary never changes a selected workout, cursor, planned session, progression proposal, compiled/scheduled Garmin workout, notification batch, or recovery interaction. It does not touch Ask Coach snapshots/prompts, CoachMessage data, morning briefings, databases outside the supplied tenant, or a private calendar. No new endpoint, fetch, sync worker, table, migration, dashboard, or UI request is part of this contract.
